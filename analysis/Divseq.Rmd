...
plot_df = div_dat@meta.data[,c("sample","nCount_RNA","nFeature_RNA")]
plot_df$exp = "regen"
ax_df = ax_srat@meta.data[,c("chem","nCount_RNA","nFeature_RNA")]
ax_df$exp = "ss"
colnames(ax_df)[1] = "sample"

plot_df = rbind(plot_df, ax_df)

plt_counts = ggplot(plot_df, aes(x = sample, y = nCount_RNA, fill = sample))+
...
plt_feats = ggplot(plot_df, aes(x = sample, y = nFeature_RNA, fill = sample))+
  facet_grid(~exp, scales = "free_x", space='free')+
  geom_violin(scale = "width")+
  theme_classic()+
  theme(axis.text = element_text(colour = "black", size = 6),
...
pdf("results/Div-seq/counts_features_div_ss.pdf", height = 7, width = 4)
print(plt_counts/plt_feats)
dev.off()
```
...
Plot ependymal clusters in pseudotime

```{r}
# load pseudotime(s)
div_dat_df = read.csv("results/RNAvelocity_Div/heatmap_gen/div_dat_df.csv", 
                      header = T, row.names = 1)

meta = ep_wpi_srat@meta.data[ep_wpi_srat@meta.data$sample_simp!="SS" &
                               ep_wpi_srat@meta.data$ctlabels %in% c("epen_clus_4", "epen_clus_3"),
                             c("clusters_used", "ctlabels")]
newrownames = rownames(meta)
newrownames = gsub("-1_1", "_2_wpi", newrownames)
newrownames = gsub("-1_2", "_4_wpi", newrownames)
newrownames = gsub("-1_3", "_6_wpi", newrownames)
newrownames = gsub("-1_4", "_8_wpi", newrownames)
newrownames = gsub("-1_5", "_12_wpi", newrownames)
newrownames = gsub("-1", "_1_wpi", newrownames)
rownames(meta) = newrownames
subdiv = div_dat_df[,c("cells","orig_pt","newpt","fate","group")]

plt_list = list()
for(g in unique(subdiv$group)){
  plot_df = merge(meta, unique(subdiv[subdiv$group==g,]), by.x = 0, by.y = 1)
  plot_df$newlabs = as.character(plot_df$clusters_used)
  plot_df$newlabs[!(plot_df$newlabs %in% c(20, 21, 8))] = as.character(plot_df$ctlabels[!(plot_df$newlabs %in% c(20, 21, 8))])
  
  plt_list[[g]] = ggplot(plot_df, aes(x = newpt, colour = newlabs))+
    geom_density()+
    labs(title = g)+
    theme_bw()
}
