...
glut_ort_l = readRDS("data/processed/integration_evo/glut_ort_l.RDS")

for(n in names(glut_ort_l)){
  print(n)
  # merge data
  srat_m = Reduce(merge, glut_ort_l[[n]])
  srat_m = AddMetaData(srat_m, metadata = unlist(lapply(names(glut_ort_l[[n]]), 
                       function(x) rep(x, ncol(glut_ort_l[[n]][[x]])))), col.name = "species")
  integr_feat = rownames(srat_m@assays$SCT@scale.data)
  
  ##### rPCA #####
  print("rPCA")
  # calculate possible missing Pearson residuals for SCTransform
  srat_ort_int = PrepSCTIntegration(glut_ort_l[[n]], anchor.features = integr_feat, verbose = T)
  ...
  srat_ort_rpca = AddMetaData(srat_ort_rpca,  metadata = unlist(lapply(names(glut_ort_l[[n]]), 
                              function(x) rep(x, ncol(glut_ort_l[[n]][[x]])))), col.name = "species")
